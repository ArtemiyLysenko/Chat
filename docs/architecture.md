...
| Messaging and history | Milestone 4.2 HTTP message send, reply, edit, delete, cursor-based history loading, forward-only unread markers, and the first real room or direct-dialog timeline UI across rooms and direct dialogs. Milestone 4.3 adds authenticated backend `/ws` fan-out for message or unread changes plus live session-revocation enforcement. Browser live wiring remains Milestone 4.4 work | PostgreSQL |
...
4. Message send, history reads, and unread updates
   Milestone 4.2 writes land in PostgreSQL first over HTTP and history reads use cursor pagination over the same store. The current implementation covers send, reply, edit, delete, newest-page load, older-page load by `before`, forward-only read-marker updates, unread badge rendering from those markers, the first real room and direct-dialog timeline UI, and Milestone 4.3 backend WebSocket fan-out for `message.created`, `message.updated`, `message.deleted`, `unread.updated`, and `session.revoked`. The browser still finishes live UI wiring in Milestone 4.4.
