...
- `2026-04-22-milestone-7-3-evidence.md` captures the verified Milestone 7 closeout, including blocked or otherwise ineligible local and federated direct-message denial over XMPP, rejection-aware federation telemetry, the Milestone 7 reviewer hardening fix for remote `forbidden` handling, and the fresh targeted plus full regression gates with the two-node compose proof.
- `2026-04-22-milestone-8-1-evidence.md` captures the verified Milestone 8.1 admin slice, including real admin authorization, classic-web Jabber connections and federation screens, compose-path admin-roster wiring, the saved admin screenshots, and the fresh targeted plus full regression gates without claiming the Milestone 8.2 load-validation closeout.
- `2026-04-22-milestone-8-2-evidence.md` captures the verified Milestone 8 closeout, including the repeatable federation load harness, the clean 50-clients-per-side two-node validation run, the saved load log and JSON summary, the saved post-run admin screenshots, the federation-telemetry concurrency fix that made the counters trustworthy under load, the Playwright fallback note, and the B3 resolution.
- `2026-04-22-review-fix-root-compose-xmpp-evidence.md` captures the review-fix verification for the root single-node compose path, including the confirmed `docker-local` root cause, the local XMPP listener proof on `localhost:5222`, the HTTP-cookie sanity check kept by the minimal fix, and the refreshed landing page copy proof.
